...
export const AssignmentSetup = () => {
  ...
  const { mutate: updateAssignment, isPending: isUpdating } = useMutation({
    ...
    onError: (error) => {
      handleApiError(error, undefined, t('settingComplaint.assignmentSetup.updateError'));
    },
  });

  const handleDelete = () => {
    if (!deleteItem?.userId) return;
    deleteAssignment(deleteItem.userId.toString());
  };

  const handleUpdate = (data: { employeeCode: string; departmentIds: string[] }) => {
    if (!editItem?.userId) return;
    updateAssignment({
      employeeCode: editItem?.userId?.toString(),
  ...
  } = useQuery<AssignmentSetupResponse, AxiosError<{ message: string }>>({
