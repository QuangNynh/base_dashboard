...
import type { FeedbackLevel2Params } from '@/types/complaint-management';
...
interface Props {
  onSubmit: (data: FeedbackLevel2Params) => void;
  onCreate: () => void;
}

const FeedbackLevel1FilterSchema = z.object({
  keyword: z.string(),
  level_1_keyword: z.string(),
  is_deleted: z.string(),
});

type FeedbackLevel1FilterData = z.infer<typeof FeedbackLevel1FilterSchema>;

export const FilterFeedbackLevel2 = ({ onSubmit, onCreate }: Props) => {
  ...
  return (
    <form onSubmit={handleSubmit(handleSearch)}>
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 mb-4'>
        ...
        <div className='flex flex-col gap-2'>
          <Label htmlFor='search'>
            {t('settingComplaint.feedbackCategorySetup.feedbackLevel2NameFilter')}
          </Label>
          <Controller
            name='keyword'
            control={control}
            render={({ field }) => (
              <InputSearch
                {...field}
                placeholder={t('settingComplaint.feedbackCategorySetup.placeholderErrorCause')}
              />
            )}
          />
        </div>
        ...
        <div className='flex items-end gap-2'>
          ...
          <Button type='button' onClick={onCreate} variant='create'>
            ...
            {t('settingComplaint.feedbackCategorySetup.addFeedbackLevel2')}
          </Button>
        </div>
      </div>
    </form>
