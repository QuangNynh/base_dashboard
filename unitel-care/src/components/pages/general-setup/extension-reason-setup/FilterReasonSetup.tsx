...
export const FilterReasonSetup = ({ onSubmit, onCreate }: Props) => {
    ...
    <form onSubmit={handleSubmit(handleSearch)}>
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-4'>
        ...
        <div className='flex flex-col gap-2'>
          <Label htmlFor='search'>{t('settingComplaint.extensionReason.reason')}</Label>
          <Controller
            name='keyword'
            control={control}
            render={({ field }) => (
              <InputSearch
                {...field}
                placeholder={t('settingComplaint.extensionReason.placeholderReason')}
              />
            )}
          />
        </div>
        ...
        <div className='flex items-end gap-2'>
          ...
          <Button type='button' onClick={onCreate} variant='create'>
            ...
            {t('settingComplaint.extensionReason.addReason')}
          </Button>
        </div>
      </div>
    </form>
